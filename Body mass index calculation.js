/*
For long period JavaScript warriors trained a lot in coding kata. But to find way warrior also need physical kata. 
...
1) value - calculated Body mass index, as a string, rounded to the first decimal.
2) category - related category, as a string (from https://en.wikipedia.org/wiki/Body_mass_index ; see below). Note: determine the category before rounding the BMI.
...
const DATA = [
    ...
    const calculateBmi = (weight,height) => {
        ...
        height = height / 100; // centimetres -> meters 

        bmi = weight / (height*height);
      
       for (let i = 0; i < DATA.length; i++) {
            if (bmi >= DATA[i][0] && bmi <= DATA[i][1]) {
                category = DATA[i][2];
                break;
            }
        }
    ...
    console.log(calculateBmi(120,175));